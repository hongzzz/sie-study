# Domain 1: Capital Markets - Topics Inventory

> **Domain Weight**: 16% of exam (~12 questions)
>
> **Last Updated**: 2025-12-26
>
> **Overall Domain Progress**: ▰▰▰▰▰▰▰▱▱▱ 75%

---

## Topics Status Tracker

| Topic | Status | File Exists | Questions | Session Date | Notes |
|-------|--------|-------------|-----------|--------------|-------|
| Market Structure (市场结构) | ✅ Taught | ❌ No | 10 | 2025-12-23 | Primary vs Secondary, NYSE vs NASDAQ - STRONG understanding |
| Offerings & Underwriting (证券发行与承销) | ✅ Taught | ✅ Yes | 10 | 2025-12-24 | Firm Commitment vs Best Efforts, Syndicate structure, SEC role - 100% verification |
| Regulatory Agencies (监管机构) | ✅ Taught | ✅ Yes | 5 | 2025-12-26 | SEC, FINRA, MSRB, SIPC - 100% accuracy on all 4 verification scenarios |
| Economic Factors (经济因素) | ⬜ Pending | ❌ No | 0 | -- | Monetary/fiscal policy, indicators |

---

## Legend | 图例

**Status**:
- ✅ **Taught** - Fully covered in session, student demonstrates good understanding
- 🟨 **Partial** - Mentioned or briefly discussed, needs deep dive
- ⬜ **Pending** - Not yet taught

**File Exists**:
- ✅ **Yes** - Topic file created in `topics/` folder
- ❌ **No** - Topic file needs to be generated

---

## Progress Summary | 进度总结

**Completed Topics**: 3 / 4 (75% taught)

**Topics with Files**: 2 / 4 (Offerings & Underwriting + Regulatory Agencies topic files created)

**Total Practice Questions**: 25 (10 Market Structure, 10 Offerings, 5 Regulatory Agencies)

**Estimated Time to Complete Domain**: 1 more session (~30-40 minutes for Economic Factors)

---

## Next Session Recommendations | 下次会话建议

### Priority 1: Complete Domain 1 (Recommended) 🎯

**Option A: Economic Factors** ⭐ Recommended
- **Why**: Last topic to complete Domain 1 (75% → 100%)
- **Prerequisites**: None
- **Estimated Time**: 1 session (30-40 min)
- **Key Learning**: Monetary vs fiscal policy, GDP, CPI, unemployment
- **Connection**: Foundation for understanding market trends
- **Achievement**: Complete first domain! 🎉

### Priority 2: Deepen Partial Understanding

**Option C: Move to Domain 2 - Debt Securities** 🎯 New Recommendation
- **Why**: Domain 1 is 75% complete; Domain 2 has highest exam weight (44%)
- **Prerequisites**: Market Structure (✅ completed), Offerings (✅ completed)
- **Estimated Time**: 1 session (40-50 min)
- **Key Learning**: Bonds, interest rates, bond pricing, types of debt securities
- **Connection**: Builds on primary/secondary market knowledge

---

## Recommended Learning Sequence | 建议学习顺序

Based on prerequisite relationships and current progress:

```
Market Structure (✅ DONE)
    ↓
[Current Position - Choose one:]
    ↓
    ├─→ Regulatory Agencies (recommended next)
    │       ↓
    │   Economic Factors
    │       ↓
    │   Types of Offerings (deep dive)
    │
    ├─→ Economic Factors
    │       ↓
    │   Regulatory Agencies
    │       ↓
    │   Types of Offerings (deep dive)
    │
    └─→ Types of Offerings (deep dive)
            ↓
        Regulatory Agencies
            ↓
        Economic Factors

✅ Domain 1 Complete → Move to Domain 2
```

**Claude's Recommendation**: Follow the first path (Regulatory Agencies → Economic Factors → Offerings) as it builds most logically.

---

## Content Generation Backlog | 内容生成待办

### Immediate (from already-taught topics):
- [x] ~~Generate topic file for Offerings & Underwriting~~ ✅ DONE 2025-12-24
  - File: domains/01-capital-markets/topics/offerings-and-underwriting.md

- [x] ~~Generate 10 practice questions for Offerings & Underwriting~~ ✅ DONE 2025-12-24
  - File: practice/questions/by-topic/offerings-and-underwriting.md

- [ ] Generate topic file for Market Structure (Session 2025-12-23)
  - Status: PENDING - needs to be created from session notes
  - Priority: MEDIUM (topic is well-mastered, but file good for review)

- [ ] Generate 10 additional practice questions for Market Structure
  - Current: 10 questions in domain-01/sample-questions.md
  - Target: 20 total (10 more needed)
  - Priority: LOW (already has 10 questions)

- [ ] Create by-topic question file: `practice/questions/by-topic/market-structure.md`
  - Priority: LOW

### Recently Completed:
- [x] ~~Topic file: Regulatory Agencies~~ ✅ DONE 2025-12-26
  - File: domains/01-capital-markets/topics/regulatory-agencies.md

- [x] ~~Practice questions for Regulatory Agencies~~ ✅ DONE 2025-12-26
  - File: practice/questions/by-topic/regulatory-agencies.md (5 questions)
  - File: practice/questions/domain-01/regulatory-agencies-questions.md

### Future (for pending topics):
- [ ] Topic file: Economic Factors
- [ ] 10 practice questions: Economic Factors

---

## Domain-Specific Notes | 领域特定笔记

### What's Working Well:
✅ Market structure concepts clicked quickly
✅ Student has strong understanding of Primary vs Secondary markets
✅ Real-world examples (Tesla, Apple) very effective
✅ Visual comparisons (NYSE vs NASDAQ table) helpful

### Common Challenges:
⚠️ Need to watch for: Confusing which regulatory agency does what
⚠️ Potential difficulty: Economic indicators can be dry - use current events

### Teaching Strategies for This Domain:
💡 Use current news for economic factors (inflation, Fed rate changes)
💡 Regulatory agencies: Create comparison table from the start
💡 Offerings: Walk through recent IPO as case study

---

## Mastery Criteria for Domain Completion | 完成标准

To consider Domain 1 fully mastered, student should be able to:

- [ ] Explain primary vs secondary market with examples ✅ (ACHIEVED)
- [ ] Identify which regulatory agency has authority in given scenarios
- [ ] Distinguish between monetary and fiscal policy with examples
- [ ] Describe Fed's role and tools (discount rate, federal funds rate, etc.)
- [ ] Explain different types of offerings (IPO, follow-on, private placement)
- [ ] Identify key economic indicators and what they measure
- [ ] Answer practice questions with 80%+ accuracy
- [ ] Apply concepts to realistic scenarios

**Current Achievement**: 2 / 8 criteria (25%)

---

## Quick Reference: Domain 1 Topics Map | 快速参考

```
DOMAIN 1: CAPITAL MARKETS (16%)
│
├─ Market Structure ✅
│  ├─ Primary Market (公司募资)
│  ├─ Secondary Market (投资者交易)
│  ├─ Market Participants
│  └─ NYSE vs NASDAQ
│
├─ Regulatory Agencies ⬜
│  ├─ SEC (federal regulator)
│  ├─ FINRA (SRO for broker-dealers)
│  ├─ MSRB (municipal securities)
│  ├─ SIPC (investor protection)
│  └─ Federal Reserve (monetary policy)
│
├─ Types of Offerings 🟨
│  ├─ IPO (initial public offering)
│  ├─ Follow-on/Secondary offerings
│  ├─ Private placements
│  └─ Shelf registrations
│
└─ Economic Factors ⬜
   ├─ Monetary Policy (Fed)
   ├─ Fiscal Policy (Government)
   └─ Economic Indicators (GDP, CPI, etc.)
```

---

## Study Resources | 学习资源

### Internal Resources:
- [Domain Overview](overview.md) - Comprehensive domain introduction
- [Progress Tracker](../../progress/sie-study-tracker.md#domain-1-knowledge-of-capital-markets-16-12题) - Overall progress
- [Session 2025-12-23 Notes](../../sessions/2025-12-23/session-notes.md) - Market Structure session
- [Practice Questions](../../practice/questions/domain-01/sample-questions.md) - 10 questions
- [Glossary](../../resources/glossary.md) - Term definitions

### Topics Files (when created):
- [Market Structure](topics/market-structure.md) - COMING SOON
- [Regulatory Agencies](topics/regulatory-agencies.md) - NOT YET CREATED
- [Types of Offerings](topics/types-of-offerings.md) - NOT YET CREATED
- [Economic Factors](topics/economic-factors.md) - NOT YET CREATED

---

## Statistics | 统计数据

**Teaching Sessions**: 4 sessions (3 topics + 1 verification)

**Total Study Time on Domain 1**: ~185 minutes (3.1 hours)

**Average Session Length**: 46 minutes

**Topics Completed per Session**: 0.75 (3 topics / 4 sessions)

**Questions Generated**: 25 (10 Market Structure, 10 Offerings, 5 Regulatory Agencies)

**Files Created**: 2 topic files (Offerings & Underwriting, Regulatory Agencies)

**Estimated Completion Date**: Week 1-2 (75% complete, 1 session remaining for Economic Factors)

---

**Last Review**: 2025-12-26
**Next Review**: After completing Economic Factors (final Domain 1 topic)

---

*This inventory is updated after each session to track teaching progress vs documentation status*
